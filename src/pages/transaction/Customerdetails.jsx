...
const Customerdetails = () => {
  return (
    <div className='flex gap-4 p-10 justify-center flex-wrap md:flex-nowrap bg-[#F9FBFC]'>
        <section className='w-5/6 md:w-[330px] xl:w-[350px] flex flex-col gap-4'>
                ...
                {customertransaction.map((data, index) =>(
                  ...
                  >
                    <tr key={index} className='row w-full flex items justify gap-4 px-6'>
                      <td className='staff flex gap-2'><span className='border-[1px] rounded-md px-[.3rem]'>{data.id}</span> {data.transactDetails}</td>
